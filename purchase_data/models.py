...
class PurchaseData(models.Model):
    ...
    item = models.ManyToManyField(Item, verbose_name="購入商品", related_name='purchase_data', blank=True)
    created_at = models.DateTimeField(verbose_name='作成日時', default=timezone.now)
    updated_at = models.DateTimeField(verbose_name='編集日時', blank=True, null=True)
...
class PurchaseDataDetail(models.Model):
    purchase_data = models.ForeignKey(
        PurchaseData,
        on_delete=models.PROTECT)

    item = models.ForeignKey(Item, verbose_name="購入商品", on_delete=models.PROTECT, default=1,)
    ...
    def __str__(self):
        return self.purchase_data.place
